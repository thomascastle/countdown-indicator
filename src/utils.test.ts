import { calcTimeDelta, zeroPad } from "./utils";
...
describe("utils", () => {
  describe("zeroPad", () => {
    ...
    it('should add three 0s is value is "" and length is 3', () => {
      expect(zeroPad("", 3)).toBe("000");
    });

    it("should not zero-padd 1 if length is 0 or 1", () => {
  ...
  describe("calcTimeDelta", () => {
    ...
    it("should return a time difference of 0s if values for start and current date are the same", () => {
      ...
      expect(
        calcTimeDelta(Date.now() + 10, { now: () => Date.now() + 10 })
      ).toEqual({
        ...defaultStats,
        completed: true,
      });
    ...
    it("should calculate the time difference by passing a date string", () => {
      ...
      expect(
        calcTimeDelta("Thu Dec 23 2017 01:38:10:456", { precision: 3 })
      ).toEqual({
        total: 31626123456,
        days: 366,
        hours: 1,
