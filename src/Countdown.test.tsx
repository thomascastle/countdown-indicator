...
describe("<Countdown />", () => {
  ...
  it("should render with zeroPadDays => 3", () => {
    wrapper = mount(
      <Countdown date={Date.now() + 10 * 86400 * 1000} zeroPadDays={3} />
    );
    expect(wrapper).toMatchSnapshot();
  });
  ...
  it("should pause => start => pause => stop and restart countdown", () => {
    ...
    // Calling pause() a second time while paused should return early
    api.pause();
    expect(api.isPaused()).toBe(true);
    expect(spies.onPause).toHaveBeenCalledTimes(1);
    ...
    // Calling stop() a second time while stopped should return early
    api.stop();
    expect(api.isStopped()).toBe(true);
    expect(spies.onStop).toHaveBeenCalledTimes(1);
    api.start();
  ...
  it("should not auto start countdown", () => {
    ...
    wrapper = mount(
      <Countdown date={countdownDate} autoStart={false} {...spies} />
    );
    const obj = wrapper.instance();
    const api = obj.getApi();
    ...
    // Calling start() a second time while started should return early
    api.start();
    expect(spies.onStart).toHaveBeenCalledTimes(1);
  });
